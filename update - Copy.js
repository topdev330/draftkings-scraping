...
async function updatePassTab(gSheetDoc, type, prtd, prtdNameArr) {
  ...
  for(let fRow of fRows) {
    ...
    if(bestMatch.bestMatch.rating < 0.3) continue;
    ...
    fRow.total = selectMember[strObj.name];
    fRow.marked = 1;
    await fRow.save();
  }
}
...
async function updateScore(sheetDoc, resData, prtd) {
  ...
  for(let type in sheetList) {
    ...
    if(type == "pass" || type == "rec" || type == "dst") {
      await updatePassTab(sheetDoc, type, prtd, prtdNameArr);
    }
